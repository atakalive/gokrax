...
- CLI commands that update state/history/spec_config (`commands/dev.py`: `cmd_transition`, `cmd_review`, `cmd_commit`, `cmd_start`, etc.; `commands/spec.py`: `cmd_spec_start`, `cmd_spec_approve`, etc.)
...
- glab API calls — issue close, title update, comment posting (`commands/dev.py`: `_update_issue_title_with_assessment`, etc.; `engine/cc.py`: `_auto_push_and_close`)
