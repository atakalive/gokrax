...
gokraxは、自動開発パイプラインによる生成コードの高精度化を目的とする。たとえ同じようなバグであっても、その重大さはプロジェクトごとに異なる。gokraxはそうしたドメイン固有のリスクに応じて、許容できない問題を含むコードが導入される頻度を減らすことを目指す。
...
- `ASSESSMENT` は設計承認後の判定ステートで、5段階のコード複雑性判定と、3段階のドメインリスク判定を行う。`--exclude-high-risk` / `--exclude-any-risk` 指定時はリスクレベルに沿って実行バッチをスキップする。
