...
- **エージェント基盤**: [openclaw](https://github.com/openclaw/openclaw) または [pi-agent-core](https://github.com/badlogic/pi-mono/tree/main/packages/agent)。設計・修正・レビューを実行する LLM エージェントの認証・プロンプト送出のために使用
...
ユーザーの環境で既に openclaw が動作しているなら、openclaw を使用するのが簡単である。そうでなければ pi のほうがセットアップは簡単である。
...
agents/
├── reviewer1/
│   ├── IDENTITY.md       # 名前など（編集する）
│   ├── INSTRUCTION.md    # 役割・ルール・レビュー指針（編集する）
│   ├── MEMORY.md         # 教訓・既知の問題（編集する）
...
実装フェーズで Claude Code CLI を使用する。バッチ実行時のオプションで ``--no-cc`` 指定すれば実装者が直接実装するので Claude Code 不使用も可能。
