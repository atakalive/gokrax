...
**→ [Quick Start](docs/quick_start_ja.md)**
...
- **エージェント基盤**: [openclaw](https://github.com/openclaw/openclaw)、[pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent)、[Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code)、または [Gemini CLI](https://github.com/google-gemini/gemini-cli)。設計・修正・レビューを実行する LLM エージェントの認証・プロンプト送出のために使用
...
- **Discord bot token**: 進捗通知用（推奨）。進捗監視用GUIは [WatcherB](https://github.com/atakalive/WatcherB) を参照

### LLM プロバイダ

gokrax は特定の LLM プロバイダに依存せず、openclaw、pi、cc、gemini が認証可能なプロバイダは使用可能：
...
最小構成で素早く試したい場合は **[Quick Start](docs/quick_start_ja.md)** を参照。

前提:
- GitLab アカウントと SSH 鍵の登録（パイプラインが自動で git push するため。手順は [Quick Start](docs/quick_start_ja.md#ssh-鍵の登録gitlab) を参照）
...
#### gemini backend 設定 (`agents/config_gemini.json`)

Gemini CLI は `thinking` や `effort` のつまみを提供しないため、設定は最小限である。ツールスコープは gokrax から設定できないので、必要であれば Gemini CLI 側で設定する。
...
キー:
- `model`: Gemini のモデル名（例: `gemini-2.5-pro`、`gemini-2.5-flash`）
- `compile-startup-md`: `true` の場合、送信のたびに `IDENTITY.md` + `INSTRUCTION.md` + `MEMORY.md` が `GEMINI.md` にコンパイルされる

テンプレートは `agents/config_gemini.example.json`。

#### openai-codex quota fallback (`agents/config_pi.json`)

PI backend で `openai-codex` を使用するエージェントに対し、ChatGPT の weekly quota が逼迫した際に別の provider (例: `github-copilot`) へ自動退避する機能。quota 枯渇によるパイプライン停止を防ぐ。

有効化するには `agents/config_pi.json` のエージェントプロファイルに fallback フィールドを追加する:
...
認証 (いずれか一方):
- **推奨**: `pi /login openai-codex` を実行。gokrax は `~/.pi/agent/auth.json` を読み、追加セットアップは不要
- **代替**: `~/.codex/auth.json` (Codex CLI 管理) が存在すれば、それが予備の認証情報として使用される。この場合 `pi /login` は不要
- 優先順位: pi auth を先に確認し、不在または期限切れの場合のみ codex auth を試行する
- **weekly** quota のみを参照する。5 時間ウィンドウは無視する
- 認証情報が未設定、または quota API に到達できない場合は元の provider で続行する (fail-open)
- 対象は `openai-codex` のみ。Anthropic 系は対象外

fallback 発火時、対象エージェントの PI session JSONL がリセットされ、fallback provider に古い context が持ち越されるのを防ぐ。cache 期限切れで元の provider に戻る際はリセットしない (Gemini fallback と同等の挙動)。
...
上記戦略(1)について、同一の初期条件（仕様書、Issue分割、作業バッチ）から生成したコードを複数の LLM で評価する実験を行った。結果、現在広く用いられている Claude Code 単体と比較して Critical、Major に分類される問題点の検出数が減少し、コード品質に改善がみられた。

詳細: [260407_report](reports/260407_report_chortal.md)
...
詳細は [docs/spec_mode_spec_ja.md](docs/spec_mode_spec_ja.md) を参照。
...
# messages_custom/{lang}/dev/design_review.py を編集
```
