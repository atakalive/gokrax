...
CCI_STARTUP_TIMEOUT_SEC: int = 120   # (deprecated, unused) 画面 handshake 撤去により未参照
CCI_BOOT_GRACE_SEC: int = 60         # claude 起動～最初の jsonl 追記までの固定猶予（秒）。self-alarm / 外部 reap の startup 項の置換。
CCI_COMPLETION_TIMEOUT_SEC: int = 1200  # cci_runner が agent 通信の 1 ターン完了を待つ上限（秒）
CCI_REAP_MARGIN_SEC: int = 120       # 外部 age ベース reap の閾値余裕分（completion+startup+margin）
CCI_ALARM_MARGIN_SEC: int = 60       # runner 内 signal.alarm の余裕分（外部 reap より短くする）
