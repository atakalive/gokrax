...
`cmd_spec_continue` ([devbar.py:1185-1188](devbar.py#L1185-L1188)) および `check_transition_spec` の auto_continue 分岐 ([watchdog.py:1556-1559](watchdog.py#L1556-L1559)) が、状態遷移のみ実行し `review_requests` のリセットと `current_reviews` のクリアを行わない。
...
```bash
devbar spec start --pj DevBar --spec docs/test.md --implementer kaneko --auto-continue
# → SPEC_REVIEW（レビュアーにプロンプト送信）
devbar spec review-submit --pj DevBar --reviewer pascal --file review_pascal.yaml
devbar spec review-submit --pj DevBar --reviewer leibniz --file review_leibniz.yaml
devbar spec review-submit --pj DevBar --reviewer dijkstra --file review_dijkstra.yaml
...
```
devbar spec revise-submit --pj PROJECT --file FILE    # SPEC_REVISE 完了報告
devbar spec plan-submit   --pj PROJECT --file FILE    # ISSUE_PLAN 完了報告
devbar spec queue-submit  --pj PROJECT --file FILE    # QUEUE_PLAN 完了報告
```

各コマンドは既存の `review-submit` ([devbar.py:1356-1471](devbar.py#L1356-L1471)) と同パターンで:
...
1. `cmd_spec_review_submit` ([devbar.py:1407-1441](devbar.py#L1407-L1441)):
   ```python
...
- `devbar spec status` ([devbar.py:1014](devbar.py#L1014)) は fallback を使用するため表示上は問題なし:
  ```python
...
| [devbar.py:1185-1188](devbar.py#L1185-L1188) | C1 (cmd_spec_continue) |
| [devbar.py:1356-1471](devbar.py#L1356-L1471) | C2 (review-submit パターン参照) |
| [devbar.py:1407-1441](devbar.py#L1407-L1441) | I1 (cmd_spec_review_submit) |
