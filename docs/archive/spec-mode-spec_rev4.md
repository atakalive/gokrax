...
**Version:** 4.0 (rev3 review reflected — Pascal/Leibniz/Dijkstra)
**Date:** 2026-03-01
...
- **[v4] SPEC_REVISEフロー単純化（Leibniz C-1 / Pascal C-1 / Dijkstra C-2）**: REVISE → 常にREVIEW。直接APPROVEDへの遷移を廃止。これにより should_continue_review の呼び出しは SPEC_REVIEW 完了後の1箇所のみに集約。current_reviews クリアタイミング問題も解消
- **[v4] DCL再読み込み（Leibniz C-2 / Dijkstra C-1）**: process()内のDCLブロックで update_pipeline(path, callback) パターンを採用。ディスクから再読み込み+state一致確認
- **[v4] マージ順序の決定性（Leibniz C-3）**: `list(set(...))` → `sorted(set(...))` で遷移先リストの順序を固定
- **[v4] extend/resume時のreview_requests初期化（Pascal C-2 / Dijkstra m-2）**: SPEC_REVIEWへ遷移する全パスでreview_requestsをpendingにリセット
- **[v4] セルフレビューリトライ上限後PAUSED（Pascal M-1 / Dijkstra M-2）**: パス2リトライ超過 → SPEC_PAUSED(paused_from="SPEC_REVISE")
- **[v4] paused判定の厳密化（Pascal M-2）**: 「パース失敗が存在しかつ有効レビュー不足」をPAUSED条件に。タイムアウトのみの場合はFAILED
- **[v4] 判定ロジック単一ソース化（Leibniz M-1）**: §2.4は要約のみ。正規の判定ロジックは§5.3の should_continue_review() に一元化
- **[v4] pipelines_dir仕様化（Leibniz M-2）**: 絶対パス記録、30日保持、ローテーション方針
- **[v4] CLIオプション優先順位表（Leibniz m-1）**: skip_review→auto_continue強制、review_only→auto_continue無視 等を明示
- **[v4] SPEC_APPROVED通知の発火元明示（Dijkstra M-3）**: _check_spec_review()のapproved分岐でdiscord_notifyを返す設計を明記
...
| pipelines_dir | str\|null | — | null | <!-- [v4] Leibniz M-2 --> レビュー原文保存先の絶対パス |
...
4. pipelines_dir を絶対パスで記録（`{project_pipelines_dir}/spec-reviews/`）
...
```
gokrax [SPEC_REVIEW] rev2 (cycle 1/5, retries: REVISE=0/3)
  spec: docs/spec-mode-spec.md
  implementer: second
  reviewers: pascal(✅ P0×1), leibniz(⏳), dijkstra(⏳)
  min_valid: 2 (full mode)
  pipelines_dir: /home/ataka/.openclaw/pipelines/gokrax/spec-reviews/
```
...
3. _reset_review_requests()（§5.4）
4. current_reviewsをクリア
5. SPEC_REVIEWに遷移
...
if pipeline.get("spec_mode") and state in SPEC_STATES:
    ...
    if action.next_state:
        action.expected_state = state  # DCL用: 現在のstateを記録
...
def should_continue_review(
    ...
    reviews = spec_config.get("current_reviews", {})
    received = {k: v for k, v in reviews.items() if v.get("raw_text") is not None}
    parsed_ok = {k: v for k, v in received.items() if v.get("parse_success")}
    parsed_fail = {k: v for k, v in received.items() if not v.get("parse_success")}
    ...
    # 2. パース失敗が存在し、かつ有効レビューが閾値未満
    #    → パース失敗がなければ単にfailed（再送で回復可能）
    #    → パース失敗があればpaused（人間の介入が必要）
...
- `gokrax spec extend`（STALLED→REVIEW）
...
4. **revise_count += 1**
5. <!-- [v4] Dijkstra M-1: 統合 --> current_reviews から §12.2 形式の要約を生成し review_history に追加。current_reviews をクリア
6. _reset_review_requests()（§5.4）
7. → **SPEC_REVIEW**（常にレビューに戻る）
...
def _apply_spec_action(pipeline_path: str, action: SpecTransitionAction, now: datetime):
    ...
    def _update(data):
        ...
        if action2.next_state and action2.next_state == action.next_state:
...
- パス: `{pipelines_dir}/` （pipeline.jsonに絶対パスで記録）
- デフォルト: `{PIPELINES_DIR}/{project}/spec-reviews/`
- 保持期間: 30日（`SPEC_REVIEW_RAW_RETENTION_DAYS`）。SPEC_DONE遷移時に期限切れファイルを削除
- 権限: 0600（owner read/write only）
...
| S-2 | gokrax.py: spec CLI（8コマンド + CLIオプション優先順位）| S-1 | +280 |
