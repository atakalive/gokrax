...
**Version:** 5.0 (rev4 review reflected — Pascal/Leibniz/Dijkstra)
**Date:** 2026-03-01
...
- **[v5] DCL適用条件の緩和（Leibniz C-1 / Pascal P-3 / Dijkstra m-2）**: `action2.next_state == action.next_state` 一致要求を削除。expected_state一致のみで十分。action2（再計算結果）を常に信頼
- **[v5] next_state=Noneアクションの適用（Dijkstra M-1）**: process()ガードを`action.next_state or action.pipeline_updates or action.send_to`に変更。タイムアウト再送・retry_counts更新が機能するように
- **[v5] extend→SPEC_REVISE直行（Pascal P-1）**: STALLED→REVIEWの空転ラウンド回避。既存current_reviewsを維持したまま改訂へ
- **[v5] timeout_at再設定の責務明示（Leibniz C-2）**: _send_spec_review_request()の事後条件として`sent_at!=None && timeout_at!=None`を断言
- **[v5] current_reviewsにtimeoutエントリ追加（Leibniz M-1）**: タイムアウトreviewerも`raw_text=None, parse_success=False`で格納。should_continue_reviewのreceived定義と整合
- **[v5] approve --force時のcurrent_reviewsアーカイブ（Pascal P-2）**: force承認ルートでも§6.3と同等のreview_history追加+クリア処理
- **[v5] pipelines_dirパス表記統一（Leibniz m-1）**: `PIPELINES_DIR/{project}/spec-reviews/`に一本化
...
if pipeline.get("spec_mode") and state in SPEC_STATES:
    ...
    # [v5] Dijkstra M-1: next_state=Noneでもupdates/sendがあれば適用
