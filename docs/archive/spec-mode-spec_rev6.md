...
**Version:** 6.0 (rev5 review reflected — Pascal/Leibniz/Dijkstra)
**Date:** 2026-03-01
...
- **[v6] pipelines_dir権限修正（Leibniz C-1）**: dir=0700, file=0600に分離。0600をディレクトリに適用するとexecuteビット欠落でアクセス不能になる致命的バグ
- **[v6] ガード条件にdiscord_notify追加（Leibniz C-2）**: 通知のみのアクションが適用されるように。将来のnudge・警告・監査通知に対応
- **[v6] last_changesフィールド追加（Pascal P-1）**: §6.3で改訂完了時にimplementerのchangesオブジェクトをspec_configに保存。rev2以降のプロンプトでchangelog_summary等を参照可能に
- **[v6] ISSUE_SUGGESTION resume時タイムアウト再計算（Pascal P-2）**: paused_from=ISSUE_SUGGESTIONの場合、issue_suggestions内のペンディングエントリも再計算
- **[v6] current_reviewsにreviewed_rev追加（Leibniz M-1）**: extend→REVISE直行時のrev紐づけ不変条件を明示
- **[v6] current_reviewsにstatusフィールド追加（Leibniz m-1）**: received/timeout/parse_failedを明示的に区別。raw_text有無に依存しない診断性
- **[v6] §4.1 extend説明修正（Dijkstra s-2）**: STALLED→REVIEW → STALLED→REVISE
...
```
<!-- [v6] Leibniz M-1 --> `reviewed_rev`: このレビュー集合が対象としたリビジョン。extend→REVISE直行時もこのrevに紐づくレビューとして扱う。review_historyへのアーカイブ時にrev番号として使用。

<!-- [v6] Leibniz m-1 --> per-reviewer `status`: `"received"` | `"timeout"` | `"parse_failed"`。should_continue_reviewはこのstatusを一次ソースとして使用:
- `received`: 応答あり + パース成功（有効レビュー）
- `timeout`: タイムアウト（raw_text=null）
- `parse_failed`: 応答あり + パース失敗
...
5. <!-- [v6] Pascal P-2 --> paused_fromがISSUE_SUGGESTIONの場合: issue_suggestions内のペンディングエントリのtimeout_atも現在時刻ベースで再計算
...
<!-- [v5] Leibniz M-1: timeoutもcurrent_reviewsに格納 -->
**timeoutの扱い:** タイムアウトしたreviewerも `current_reviews` に格納する:
```json
"pascal": {
  "verdict": null,
  "items": [],
...
```
これにより should_continue_review() の `received`（raw_text != None）から自然に除外され、`parsed_fail`にも含まれない（raw_text=Noneのため）。全員タイムアウト → received=0 → "failed" となる。
...
def should_continue_review(
    ...
    # [v6] Leibniz m-1: statusを一次ソースとして使用
    reviewer_entries = {k: v for k, v in reviews.items() if isinstance(v, dict) and "status" in v}
...
3. last_commit, current_rev, rev_index 更新。<!-- [v6] Pascal P-1 --> implementerのchangesオブジェクトを`last_changes`に保存（rev2以降のプロンプトで`{changelog_summary}`, `{added_lines}`, `{removed_lines}`として参照）
4. **revise_count += 1**
