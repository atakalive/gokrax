...
3. レビュー結果が返ってくるのを待つ（返ってこないレビュアーを催促）
...
[devbar spec start]
  ...
  ISSUE_PLAN
        ...
        ▼
  QUEUE_CREATE ──────────────── [--no-queue 指定時: ここで終了]
        │ (devbar-queue.txt 生成)
        ▼
  SPEC_DONE
        │ (M 確認)
...
| `QUEUE_CREATE` | Issue 依存関係を分析し、devbar-queue.txt にバッチ行を生成 | ISSUE_PLAN 完了 | queue 書き込み完了 |
| `SPEC_DONE` | 全工程完了。M の確認待ち | QUEUE_CREATE 完了 | M が確認 → IDLE |

### 2.3 既存ステートとの共存

spec mode のステート（`SPEC_*`, `ISSUE_*`, `QUEUE_CREATE`, `SPEC_DONE`）は既存の実装フローステート（`DESIGN_PLAN`, `IMPLEMENTATION` 等）と**排他**。同一プロジェクトで同時に両方は動かない。
...
devbar spec start --pj PROJECT --spec SPEC_PATH --implementer AGENT_ID
                  [--no-issue] [--no-queue] [--max-cycles N]
...
2. `commit` ハッシュを pipeline.json に記録
3. `current_rev` を更新
4. `review_history` にラウンド結果を追加
...
QUEUE_CREATE 完了後、SPEC_DONE に遷移。Discord #dev-bar に完了通知を送信:
...
| QUEUE_CREATE | `SPEC_REVISE_TIMEOUT_SEC` | spec_implementer に催促 | M に通知 |
...
| QUEUE_CREATE 完了 | `[Spec] {project}: キュー生成完了 ({n} batches). M 確認待ち` |
...
| S-6 | spec_issue.py: ISSUE_SUGGESTION + ISSUE_PLAN + QUEUE_CREATE | S-4 | +250行 |
| S-7 | notify.py: spec mode 通知 | S-4 | +80行 |
| S-8 | watchdog.py: ISSUE_* + QUEUE_CREATE ステート処理 | S-6, S-7 | +100行 |
| S-9 | 統合テスト | S-8 | +400行 |
...
- **spec mode 自身のブートストラップ**: spec mode が完成したら、この spec 自体を spec mode で改訂する
