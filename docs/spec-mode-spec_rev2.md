...
- **[v2] MAX_CYCLES 到達時の強制承認廃止（Pascal C-2）**: P1 以上が残存する場合は SPEC_APPROVED ではなく `SPEC_STALLED` に遷移し、M の明示的介入を必須化。`devbar spec approve --force` でのみ突破可能
...
devbar に **spec mode** を追加し、上記 2〜9 を自動化する:
...
- devbar-queue.txt へのバッチ行生成

### 1.3 スコープ

**スコープ内:** spec レビューサイクルの自動化、Issue 分割の半自動化、キュー生成の自動化

**スコープ外:** spec 叩き台の自動生成、devbar 実装フローとの直接接続、ブートストラップ
...
| `QUEUE_PLAN` | devbar-queue.txt生成 | PLAN完了 | 生成完了 → DONE |
...
`devbar spec start` は pipeline.json を flock で排他ロック → `spec_mode = true` を atomic に設定。`spec_mode = true` の間、既存 `devbar start` / `devbar transition` はエラー。IDLE遷移時に `spec_mode = false` にクリア。
...
```
DevBar [SPEC_REVIEW] rev2 (cycle 1/5, retries: 0/3)
...
M が `devbar spec continue` 実行後に遷移。レビュアーにIssue分割案を問い合わせ。
...
| S-2 | devbar.py: spec CLI | S-1 | +250 |
