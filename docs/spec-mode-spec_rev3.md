...
- **[v3] STALLED→REVIEW時のrevise_count扱い（Leibniz C-5）**: `devbar spec extend --pj X --cycles N` でmax_revise_cyclesを増加。revise_countはリセットしない
...
- **[v3] reviews/コミット規約（Leibniz M-2）**: devbarがcommit。コミットメッセージ規約固定。レビュー原文はrepo外（pipelines/）に保存し、repoにはサマリーのみ
...
- **[v3] STALLED→REVIEW CLI定義（Dijkstra m-1）**: `devbar spec extend`コマンド
...
- **[v3] --no-issue → --spec-only リネーム（M指示）**: 直感的な命名に変更。レビューサイクルのみ実行（Issue分割・キューをスキップ）
- **[v3] --auto-continue フラグ追加（M指示）**: SPEC_APPROVED後にM確認なしでISSUE_SUGGESTIONへ自動進行。デフォルトはM確認待ち（既存devbarのマージサマリー→M確認パターンを踏襲）
...
[devbar spec start]
  ...
  │     │ (P1以上なし)
  │     ▼
  │   SPEC_APPROVED ──── [--spec-only] ───→ SPEC_DONE
  │     │
  │     │ [devbar spec continue]
  │     ▼
  │   ISSUE_SUGGESTION
  │     │
  │     ▼
...
| `SPEC_APPROVED` | 改訂サイクル完了 | auto_continue → 自動でISSUE_SUGGESTION / デフォルト → M確認待ち（`spec continue`）/ --spec-only → DONE |
...
| --skip-review | --spec-only | --no-queue | --auto-continue | 開始 | 終了 | M確認 | 用途 |
|---|---|---|---|---|---|---|---|
| ✗ | ✗ | ✗ | ✗ | REVIEW | DONE | APPROVED時 | 全工程（デフォルト）|
| ✗ | ✗ | ✗ | ✓ | REVIEW | DONE | なし | 全工程（自動進行）|
| ✗ | ✓ | — | — | REVIEW | DONE | なし | レビューのみ |
| ✗ | ✗ | ✓ | ✗ | REVIEW | DONE | APPROVED時 | Issue起票まで |
| ✓ | ✗ | ✗ | ✗ | APPROVED | DONE | APPROVED時 | Issue化+キュー |
| ✓ | ✗ | ✓ | ✗ | APPROVED | DONE | APPROVED時 | Issue起票のみ |
| ✓ | ✓ | — | — | **エラー** | — | — | 無意味 |
...
| spec_only | bool | — | false | レビューサイクルのみ（Issue分割・キュースキップ） |
...
| --spec-only | spec_config.spec_only | bool |
...
devbar spec start --pj PROJECT --spec SPEC_PATH --implementer AGENT_ID
                  [--spec-only] [--no-queue] [--skip-review]
                  [--max-cycles N] [--review-mode MODE] [--model MODEL]
                  [--auto-continue]
```

**前提条件:** IDLE状態、specファイル存在、implementer利用可能、`--skip-review --spec-only`はエラー
...
```
DevBar [SPEC_REVIEW] rev2 (cycle 1/5, retries: REVIEW=0/3)
...
5. それ以外 → SPEC_REVISE（有効レビューで改訂サイクルへ）
...
M が `devbar spec continue` 実行後に遷移。
...
def check_transition_spec(
    ...
    if state == "SPEC_REVIEW":
        ...
        if spec_config.get("spec_only"):
            return SpecTransitionAction(next_state="SPEC_DONE",
                discord_notify=f"[Spec] spec承認完了（--spec-only）")
        ...
        # デフォルト: M確認待ち（devbar spec continue で進行）
        return SpecTransitionAction(next_state=None,
            discord_notify=f"[Spec] spec承認完了。`devbar spec continue` でIssue分割へ進行")
...
| S-2 | devbar.py: spec CLI（8コマンド）| S-1 | +280 |
