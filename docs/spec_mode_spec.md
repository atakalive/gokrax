...
- `MIN_VALID_REVIEWS`: review_modeに追随。full=4, standard=3, lite=2, min=1, lite3=3, lite3_woGoogle=3, skip=0

> **⚠️ 既知の制限事項:** `MIN_VALID_REVIEWS_BY_MODE` には `cheap`, `lite3_woOpus`, `lite3_woOpenAI` のエントリが存在しない。これらのモードで `should_continue_review()` を呼ぶと `ValueError` が発生する。ただし `gokrax spec start --review-mode` の argparse choices は `["full", "standard", "lite", "min"]` に制限されているため、通常の使用では問題にならない。
...
- `verdict` ∈ {"APPROVE", "P0", "P1", "P2"}（必須、null不可）
...
MIN_VALID_REVIEWS_BY_MODE = {
    "full": 4, "standard": 3, "lite": 2, "min": 1,
    "lite3": 3, "lite3_woGoogle": 3, "skip": 0,
}
...
**--review-mode の選択肢制約:** spec modeでは `--review-mode` の argparse choices は `["full", "standard", "lite", "min"]` の4種のみ。通常モードで利用可能な他のモード（lite3, lite3_woGoogle等）はspec startでは指定不可。
