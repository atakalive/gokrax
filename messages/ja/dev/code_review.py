...
def review_request(
    ...
    **_kw,
    ...
    """
    return (
        f"[gokrax] {project}: コードレビュー依頼{comment_line}\n\n"
        f"{todo_header}\n\n{guidance}\n\n{body}{completion}"
    )
...
def file_review_request(
    ...
    **_kw,
    ...
    """
    return (
        f"[gokrax] {project}: コードレビュー依頼（{n}件）\n\n"
...
def guidance_code(**_kw) -> str:
    ...
    return (
        ...
        "\n\nverdict の選び方:\n"
        "- P0/P1/P2 いずれかの指摘がある場合、最も重い指摘に合わせた verdict を使え\n"
        "- APPROVE は指摘ゼロの場合のみ使用可\n"
        "- 「APPROVE だが P2 あり」は禁止。P2 指摘があるなら --verdict P2 にせよ"
    )
