...
def transition(
    ...
    **_kw,
    ...
    """設計修正フェーズの指示メッセージ（watchdog.py get_notification_for_state DESIGN_REVISE）。"""
    return (
        f"[gokrax] {project}: 設計修正フェーズ\n"
        f"{comment_line}"
        f"対象Issue: {issues_str}\n"
        f"{p2_note}"
        f"【手順】\n"
        f"1. {fix_label}を読み、修正後の Issue 本文を一時ファイル /tmp/gokrax-{project}-N.md に書き出す。\n"
        f"   指摘の確認: `{GOKRAX_CLI} get-comments --pj {project} --issue N`\n"
        f"2. 次のコマンドで Issue 本文を反映する:\n"
...
def notify_revise_summary(project: str, revise_lines: str, q_prefix: str = "", **_kw) -> str:
    """REVISE遷移時のP0サマリー通知。revise_lines は組み立て済み。"""
    return f"{q_prefix}[{project}] REVISE対象:\n{revise_lines}"
