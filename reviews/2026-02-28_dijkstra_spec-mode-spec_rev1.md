...
**修正案:** `devbar spec done --pj X` コマンドを追加、または自動 IDLE 遷移。

### M-6: 排他制御の詳細が不十分 (§2.3)

IDLE 状態で `devbar start` と `devbar spec start` が同時に呼ばれた場合の競合防止策なし。spec_mode=true の解除タイミング不明。

**修正案:** `devbar spec start` が atomic に spec_mode=true を設定し、spec_mode=true 時に既存コマンドをエラーにする規則を明記。
