...
# 設定
# ============================================================
REPO_DIR="/mnt/s/wsl/work/project/DevBar"
...
declare -A FILE_RENAMES=(
    # Tier 1: リポ内
    ["$REPO_DIR/devbar.py"]="$REPO_DIR/gokrax.py"
    ["$REPO_DIR/devbar-queue.txt"]="$REPO_DIR/gokrax-queue.txt"
    # Tier 2: shared
    ["$SHARED_DIR/pipelines/devbar.json"]="$SHARED_DIR/pipelines/gokrax.json"
    ["$SHARED_DIR/devbar-state.json"]="$SHARED_DIR/gokrax-state.json"
    ["$SHARED_DIR/devbar-metrics.jsonl"]="$SHARED_DIR/gokrax-metrics.jsonl"
)

# シンボリックリンク更新
declare -A SYMLINK_UPDATES=(
    ["$SHARED_DIR/bin/devbar"]="$SHARED_DIR/bin/gokrax"
)

# ディレクトリリネーム（最後に実行）
# 注: NTFSはcase-insensitive。DevBar→gokrax は一時名経由が必要
...
show_text_changes() {
    ...
    while IFS= read -r line; do
        ...
        has_match=true
    done < <(grep -n -E "DEVBAR|DevBar|dev-bar|devbar" "$file" 2>/dev/null || true)
    $has_match
}
...
echo -e "${CYAN}================================================${NC}"
echo -e "${CYAN}  DevBar → gokrax リネームスクリプト${NC}"
echo -e "${CYAN}================================================${NC}"
if $DRY_RUN; then
...
# Tier 1: リポ本体
log_section "Tier 1: DevBarリポ ($REPO_DIR)"
while IFS= read -r file; do
    if grep -qE "DEVBAR|DevBar|dev-bar|devbar" "$file" 2>/dev/null; then
        rel="${file#$REPO_DIR/}"
...
for target in \
    "$SHARED_DIR/pipelines/devbar.json" \
    "$SHARED_DIR/devbar-state.json" \
    "$SHARED_DIR/devbar-metrics.jsonl" \
    "$SHARED_DIR/knowledge.md" \
    ; do
    if [[ -f "$target" ]] && grep -qE "DEVBAR|DevBar|dev-bar|devbar" "$target" 2>/dev/null; then
...
for old in "${!SYMLINK_UPDATES[@]}"; do
    ...
    if [[ -L "$old" ]]; then
        target=$(readlink "$old")
        # シンボリックリンクの参照先も更新
        new_target="${target//devbar/gokrax}"
        new_target="${new_target//DevBar/gokrax}"
...
# --- Step 4: /tmp ファイル名 ---
log_section "Step 4: /tmp パス（情報のみ — 再起動時に自動更新）"
echo "  以下の /tmp ファイルはwatchdog再起動時に新名で作られる:"
echo "    /tmp/devbar-watchdog-loop.lock → /tmp/gokrax-watchdog-loop.lock"
echo "    /tmp/devbar-watchdog-loop.pid  → /tmp/gokrax-watchdog-loop.pid"
echo "    /tmp/devbar-watchdog.lock      → /tmp/gokrax-watchdog.lock"
echo "    /tmp/devbar-watchdog.log       → /tmp/gokrax-watchdog.log"
echo "    /tmp/devbar-review/            → /tmp/gokrax-review/"

# --- Step 5: ディレクトリリネーム ---
log_section "Step 5: ディレクトリリネーム"
log_action "RENAME" "$REPO_DIR → $REPO_DIR_NEW (via tmp)"
if ! $DRY_RUN; then
    # NTFSはcase-insensitive。DevBar→gokrax は直接mvで可能（異なる文字列なので）
    # ただし安全のため一時名経由
...
if crontab -l 2>/dev/null | grep -q "devbar"; then
    log_action "CRONTAB" "devbar → gokrax"
    if ! $DRY_RUN; then
        crontab -l 2>/dev/null | sed 's/devbar/gokrax/g; s/DevBar/gokrax/g' | crontab -
    else
        echo "  現在のcrontabエントリ:"
        crontab -l 2>/dev/null | grep "devbar" | while IFS= read -r line; do
            echo -e "    ${RED}-${NC} $line"
            echo -e "    ${GREEN}+${NC} $(echo "$line" | sed 's/devbar/gokrax/g; s/DevBar/gokrax/g')"
        done
    fi
else
    echo "  crontab に devbar エントリなし（スキップ）"
fi

# --- Step 7: GitLab remote更新 ---
log_section "Step 7: GitLab remote（手動）"
echo "  以下を手動で実行:"
echo "    1. GitLabでリポ名変更: atakalive/devbar → atakalive/gokrax"
echo "    2. git remote set-url origin git@gitlab.com:atakalive/gokrax.git"
echo "    3. leibniz workspace内のdevbarクローンも更新"
echo "    4. DEVBAR_DRY_RUN 環境変数を使用している箇所があれば GOKRAX_DRY_RUN に変更"
echo "    5. エージェント workspace の MEMORY.md / memory/*.md 内の devbar 参照は後日対応"
...
if $DRY_RUN; then
    ...
    echo "    5. DEVBAR_DRY_RUN 環境変数の外部参照を GOKRAX_DRY_RUN に更新"
    echo "    6. エージェント workspace の devbar 参照を後日更新"
fi
