...
class TestWatchdogNudgeWiring:
    ...
    def test_design_review_nudge_contains_refresher_and_instructions(
        ...
        assert "レビュー観点" in msg or "Review guidelines" in msg, "guidance present"
        assert "数理的に精確か" in msg or "mathematically precise" in msg, "guidance_design distinguisher"
    ...
    def test_code_review_nudge_contains_diff_refresher(
        ...
        assert "Anonymous review" in msg, "anonymity constraint"
        assert "レビュー観点" in msg or "Review guidelines" in msg, "guidance present"
        assert "仕様通りに実装" in msg or "consistent with the design" in msg, "guidance_code distinguisher"
