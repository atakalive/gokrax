...
class TestDefaultReviewModeValidation:
    """DEFAULT_REVIEW_MODE が REVIEW_MODES に含まれることを検証"""

    def test_default_review_mode_in_review_modes(self):
        from config import DEFAULT_REVIEW_MODE, REVIEW_MODES
        assert DEFAULT_REVIEW_MODE in REVIEW_MODES

    def test_default_review_mode_validation_fails(self, monkeypatch):
        import config
        monkeypatch.setattr(config, "DEFAULT_REVIEW_MODE", "nonexistent")
        with pytest.raises(ValueError, match="DEFAULT_REVIEW_MODE"):
            config._validate_review_modes(config.REVIEW_MODES, config.REVIEWERS)
