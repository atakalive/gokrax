...
# ---------------------------------------------------------------------------
# A2: issues_found pending feedback, send BUSY → counter rolled back + busy_since set
# ---------------------------------------------------------------------------

class TestA2Busy:
    def test_a2_busy_rolls_back_retries_and_sets_busy_since(self, tmp_pipelines):
        sc = {
            "spec_implementer": "impl1",
            "spec_path": "docs/spec.md",
            "current_rev": "2",
            "_issues_found_pending_feedback": "Fix these:\n- x",
        ...
        # Pre-compute action from _check_spec_revise to supply to _apply_spec_action
        ...
        # Retry counter rolled back: net 0 (incremented then decremented)
        assert sc_out.get("_issues_found_send_retries", 0) == 0
        # Pending feedback restored via send_failure_rollback
        assert sc_out.get("_issues_found_pending_feedback") == "Fix these:\n- x"
        # busy_since_key initialized
        assert sc_out.get("_issues_found_busy_since") is not None

    def test_a2_ok_clears_busy_since(self, tmp_pipelines):
        sc = {
            "spec_implementer": "impl1",
            "spec_path": "docs/spec.md",
            "current_rev": "2",
            "_issues_found_pending_feedback": "Fix these:\n- x",
            "_issues_found_send_retries": 0,
            "_issues_found_busy_since": _now().isoformat(),
            "_revise_sent": _now().isoformat(),
            "review_requests": {},
            "current_reviews": {"entries": {}},
            "retry_counts": {},
